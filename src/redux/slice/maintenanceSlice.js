...
export const maintenanceHistoryData = createAsyncThunk(
    ...
    async ({ page = 1, search = "", startDate = "", endDate = "", name = 'all', division = 'all', departmentFilter = 'all', approvalStatus = 'all', unitFilter = 'all', limit = 50, usePlannedDate = false } = {}) => {
        return await fetchMaintenanceDataForHistory(page, search, startDate, endDate, name, division, departmentFilter, approvalStatus, unitFilter, limit, usePlannedDate);
    }
);
...
export const uniqueMaintenanceTaskData = createAsyncThunk(
    ...
    async ({ page = 0, pageSize = 50, nameFilter = "", freqFilter = "", append = false, userRole = "", userDept = "", userDiv = "", userName = "", deptFilter = "", divFilter = "", search = "" }) => {
        const result = await fetchUniqueMaintenanceData(page, pageSize, nameFilter, freqFilter, userRole, userDept, userDiv, userName, deptFilter, divFilter, search);
