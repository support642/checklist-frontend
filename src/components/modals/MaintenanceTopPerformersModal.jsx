...
const MaintenanceTopPerformersModal = ({ 
  isOpen, 
  onClose, 
  performers = [],
  startDate,
  ...
  const handleDownloadPDF = () => {
    ...
    // Sub-header (Purple/Blue Banner for Maintenance)
    const subHeaderY = 30;
    doc.setFillColor(233, 213, 255); // Purple light tint
    doc.rect(marginX, subHeaderY, usableW, 8, 'F');
    doc.rect(marginX, subHeaderY, usableW, 8);
    doc.setFontSize(11);
    doc.setFont("helvetica", "bold");
    const subHeaderTitle = isCustomRange 
      ? "MAINTENANCE TOP PERFORMERS RANKING (DATE RANGE)" 
      : `MAINTENANCE TOP PERFORMERS - ${monthLabel.toUpperCase() || 'CURRENT MONTH'}`;
    doc.text(subHeaderTitle, centerX, subHeaderY + 5.5, { align: "center" });

    // Period / Date Range text
    if (startDate && endDate) {
      ...
      const dateLabel = isCustomRange
        ? `Maintenance performance chart is based on selected dates: ${formatDateForDisplay(startDate)} to ${formatDateForDisplay(endDate)}`
        : `Monthly Maintenance Performance Chart for ${monthLabel} (Period: ${formatDateForDisplay(startDate)} to ${formatDateForDisplay(endDate)})`;
    ...
    const filenamePrefix = isCustomRange ? "DateRange" : (monthLabel || "Monthly").replace(/\s+/g, "_");
    doc.save(`Maintenance_Top_Performers_${filenamePrefix}_${new Date().toLocaleDateString('en-CA')}.pdf`);
  };
    ...
    <div className="fixed inset-0 z-[3000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white w-full max-w-7xl h-auto lg:h-[95vh] flex flex-col rounded-xl overflow-hidden shadow-2xl border border-gray-300 animate-in zoom-in-95 duration-200">
        ...
        <div className="hidden lg:flex flex-col flex-1 overflow-hidden">
          ...
          {/* Sub-header */}
          <div className="bg-purple-100 border-b border-purple-200 py-3 shrink-0 flex items-center justify-center relative">
            <h2 className="text-2xl font-bold text-purple-900 text-center uppercase tracking-widest flex items-center gap-2">
              <Trophy className="h-6 w-6 text-amber-600" />
              {isCustomRange ? "Maintenance Top Performers Ranking" : "Maintenance Monthly Performance Chart"}
            </h2>
            <button 
              onClick={onClose}
              className="absolute right-4 p-1.5 rounded-full hover:bg-purple-200 text-purple-700 hover:text-purple-900 transition-colors cursor-pointer"
              title="Close"
            >
              <X size={20} />
            </button>
          ...
          <div className="bg-[#F8F9FA] border-b border-gray-300 py-3 px-6 text-sm text-gray-600 flex justify-between items-center shrink-0">
            <div className="flex flex-col gap-1">
              <div>
                Showing maintenance <span className="font-bold text-purple-700">Top Performers Ranking</span> based on on-time maintenance task completion score.
              </div>
              {startDate && endDate && (
            ...
            <table className="w-full border-collapse border border-gray-300 text-[14px]">
              <thead>
                <tr className="bg-purple-50 border-b border-purple-200 text-purple-900">
                  ...
                  <th className="border border-purple-200 p-3 font-bold w-[180px] text-center">Assigned/Done</th>
                  <th className="border border-purple-200 p-3 font-bold w-[150px] text-center">On-Time Score</th>
                  <th className="border border-purple-200 p-3 font-bold w-[220px] text-center">Status Badge</th>
                </tr>
              </thead>
              <tbody>
                {performers.length > 0 ? (
                  performers.map((staff, idx) => {
                    ...
                    return (
                      <tr key={`${staff.name}-${idx}`} className={`hover:bg-purple-50/30 transition-colors border-b border-gray-200 ${rowBg}`}>
                        ...
                        <td className="border border-gray-300 p-3 text-center font-bold text-purple-700">
                          {onTimePct}%
                        </td>
          ...
          <div className="p-4 bg-gray-50 flex items-center justify-end border-t border-gray-300 shrink-0">
            <button 
              ...
              className="flex items-center gap-2 px-6 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-lg transition-all font-bold shadow-md active:scale-95 mr-3 cursor-pointer"
            >
              <Download size={18} />
              Download PDF
            </button>
            <button 
              onClick={onClose}
              className="px-8 py-2 bg-gray-800 text-white rounded-lg hover:bg-black font-bold transition-all shadow-md active:scale-95 flex items-center gap-2 cursor-pointer"
            >
              <X size={18} />
              Close Chart
            </button>
        ...
        <div className="lg:hidden flex flex-col w-full max-w-sm mx-auto bg-white rounded-xl shadow-2xl overflow-hidden border border-gray-200">
          ...
          <div className="p-8 space-y-6">
            <div className="space-y-4 text-center">
              <div>
                <p className="text-xs font-semibold text-purple-600 uppercase tracking-widest">Chart Scope</p>
                <p className="text-base font-bold text-gray-800 leading-tight">
                  Maintenance Performance Ranking
                </p>
              </div>

              {startDate && endDate && (
                <div className="bg-purple-50 p-4 rounded-xl border border-purple-100 space-y-1">
                  <p className="text-[10px] font-bold text-purple-600 uppercase tracking-wider">Date Period</p>
            ...
            <div className="grid grid-cols-1 gap-3">
              <button 
                onClick={handleDownloadPDF}
                className="flex items-center justify-center gap-3 px-6 py-4 bg-amber-600 text-white rounded-xl hover:bg-amber-700 transition-all font-bold shadow-lg active:scale-95 text-sm uppercase tracking-wider cursor-pointer"
              >
                <Download size={20} />
                Download PDF
              </button>
            ...
            <button 
              onClick={onClose}
              className="w-full py-3 text-gray-500 font-semibold hover:text-gray-800 transition-colors uppercase tracking-widest text-xs flex items-center justify-center gap-2 cursor-pointer"
            >
              <X size={14} />
              Cancel / Close
            </button>
...
export default MaintenanceTopPerformersModal;
