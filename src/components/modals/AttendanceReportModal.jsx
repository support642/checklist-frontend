...
const AttendanceReportModal = ({
    ...
    const handleDownloadPDF = () => {
        ...
        const subHeaderTitle = isCustomRange
            ? "EMPLOYEE ATTENDANCE REPORT (DATE RANGE)"
        ...
        if (startDate && endDate) {
            ...
            const dateLabel = isCustomRange
                ? `Attendance report is based on the selected dates: ${formatDateForDisplay(startDate)} to ${formatDateForDisplay(endDate)}`
                : `Monthly Attendance Report for ${monthLabel} (Period: ${formatDateForDisplay(startDate)} to ${formatDateForDisplay(endDate)})`;

            doc.text(dateLabel, centerX, subHeaderY + 12, { align: "center" });
        }

        // 3. Info Table
        const tableHeader = ["Rank", "Employee", "Designation", "Department", "Division", "Present", "Absent", "Status Badge"];
        ...
        doc.save(`Employee_Attendance_Report_${filenamePrefix}_${new Date().toLocaleDateString('en-CA')}.pdf`);
    };
                ...
                <div className="hidden lg:flex flex-col flex-1 overflow-hidden">
                    ...
                    <div className="bg-[#D9EAF7] border-b border-gray-300 py-3 shrink-0 flex items-center justify-center relative">
                        <h2 className="text-2xl font-bold text-gray-800 text-center uppercase tracking-widest">
                            {isCustomRange ? "Attendance Report (Date Range)" : "Monthly Attendance Report"}
                        </h2>
                        <button
                            onClick={onClose}
                        ...
                        <div className="flex flex-col gap-1">
                            <div>
                                Showing {isCustomRange ? (
                                    <>
                                        overall <span className="font-bold text-blue-600">Attendance Report</span> based on custom date range
                                    </>
                                ) : (
                                    <>
                                        monthly <span className="font-bold text-blue-600">Attendance Report</span> for <span className="font-bold text-blue-600">{monthLabel || "this month"}</span>
                                    </>
                                )}
                            </div>
                            {startDate && endDate && (
                        ...
                        <table className="w-full border-collapse border border-gray-300 text-[14px]">
                            <thead>
                                <tr className="bg-[#E9F2E9] border-b border-gray-300">
                                    ...
                                    <th className="border border-gray-300 p-3 font-bold w-[150px] text-center">Present</th>
                                    <th className="border border-gray-300 p-3 font-bold w-[150px] text-center">Absent</th>
                                ...
                                {performers.length > 0 ? (
                                    performers.map((staff, idx) => {
                                        ...
                                        return (
                                            <tr key={`${staff.name}-${idx}`} className={`hover:bg-gray-50 transition-colors border-b border-gray-200 ${rowBg}`}>
                                                ...
                                                <td className="border border-gray-300 p-3 text-center text-green-700 font-bold">
                                                    {present}
                                                </td>
                                                <td className="border border-gray-300 p-3 text-center text-red-600 font-bold">
                                                    {absent}
                                                </td>
                                    ...
                                    <tr>
                                        <td colSpan="8" className="p-8 text-center text-gray-400 italic">
                                            No employees meet the attendance criteria
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                ...
                <div className="lg:hidden flex flex-col w-full max-w-sm mx-auto bg-white rounded-xl shadow-2xl overflow-hidden border border-gray-200">
                    ...
                    <div className="p-8 space-y-6">
                        <div className="space-y-4 text-center">
                            <div>
                                <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Report Type</p>
                                <p className="text-base font-bold text-gray-800 leading-tight">
                                    {isCustomRange ? "Date Range Attendance Report" : `Attendance Report for ${monthLabel}`}
                                </p>
                            </div>
